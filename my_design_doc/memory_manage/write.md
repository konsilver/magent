...
Mem0记忆主要分为以下三个模块（一般不会联合使用或互相重叠），每个模块标注了自己以什么方式存储：
...
写内容：user-Urgent与mem字段的合并（如果有冲突的部分，以Urgent字段为正确，如果有重叠则覆盖，带时间戳）

# Plan（KV + Graph）
planner制定出并执行完的计划，计划执行结果分为success（通过QA check，这里包括流程中触发了REDO\REPLAN等，只要走到了最后step结果被QAcheck通过就算）、fail（触发了全局计划重置或在执行前用户没有输入“确认执行”，而是输入“重新规划+建议”），存的是可复用的规划结构 + 优化建议

适用 agent：planner、warmup agent

写时机：plan执行完后，执行后异步写Memory（三种结果情况都写）
写内容：
a. 调用LLM抽取plan的decomposition schema，输入：context 输出:注意一定要告诉LLM，你尽量的提取具有参考性的计划的结构骨架，而非具体任务的拆解方案，任务精炼分解策略(一句话)+抽象节点（去除实现细节）+节点依赖关系+任务type
b. KV模式存储：
        {
            "plan_id": "uuid",
            "embedding": [...],
...
c. Graph模式存储：
    ...
    失败样本：
        ...
        Suggestion
        Suggestion示例：
            description = "需要添加需求分析阶段"

d.结合context按照上述形式存储到memory两种模式中，他们之间连接的桥梁是plan_id
...
相关背景：在整个任务执行完之后，我们在context上可以看到每个子任务是怎么做的，遍历每个step，如果这个step子任务被QA check为成功，则调用LLM先自问“这个信息，能不能在“不同任务但相似 step”中复用？”如果认为能，则：结合任务的tool_use轨迹（在global中）且如果这个step曾经被QA判定REDO（注意不是REPLAN），结合QA在context填充的suggestion字段，让LLM提取能作为后面相似任务的经验的部分存入memory（KV模式）
