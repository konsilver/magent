...
相关背景：在整个任务执行完之后，我们在context上可以看到每个子任务是怎么做的，遍历每个step，如果这个step子任务被QA check为成功，则调用LLM先自问“这个信息，能不能在“不同任务但相似 step”中复用？”如果认为能，则：结合任务的tool_use轨迹且如果这个step曾经被QA判定REDO（不是REPLAN），结合QA在context填充的suggestion字段，让LLM提取能作为后面相似任务的经验的部分存入memory（KV模式）
