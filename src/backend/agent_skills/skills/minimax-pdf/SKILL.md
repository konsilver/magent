---
name: minimax-pdf
display_name: PDF文档生成
description: "当用户需要生成高质量PDF文档、填充PDF表单或重新排版文档时使用，如\"生成一份PDF报告\"\"帮我填这个PDF表格\"\"把这份文档排版成PDF\"\"做一份简历PDF\"。支持从零生成、表单填充、已有文档重排版三种模式，输出印刷级质量。"
version: 1.0
tags: document-generation,pdf,office
---
...
**通过 run_skill_script 调用时**，使用 `input_files` 传递 content.json：
- `params`: `'run --title "Q3 Strategy Review" --type proposal --content content.json --out report.pdf'`
- `input_files`: `'{"content.json": "[{\"type\":\"h1\",\"text\":\"Summary\"}, ...]"}'`

短内容也可用 `--content-json` 直接内联到 params 中（无需 input_files）：
- `params`: `'run --title "报告" --type report --content-json \'[...]\' --out output.pdf'`
...
**通过 run_skill_script 调用时**，用户上传的文件通过 `artifact:` + 对话上下文中的 file_id 引用传入：
- `params`: `'reformat --input source.docx --title "报告" --type report --out output.pdf'`
- `input_files`: `'{"source.docx": "artifact:ua_abc123def456"}'`（ua_abc123def456 取自对话中 file_id 的实际值）

**Supported input formats:** `.md` `.txt` `.docx` `.pdf` `.json`
