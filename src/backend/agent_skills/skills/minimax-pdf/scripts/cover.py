...
def _base_css(t: dict) -> str:
    ...
    cjk_fallback = "'WenQuanYi Zen Hei', 'Microsoft YaHei', 'PingFang SC', 'Noto Sans CJK SC'"
    return f"""
{_gfonts_import(t)}
@font-face {{
    font-family: 'CJK Fallback';
    src: local('WenQuanYi Zen Hei'), local('Microsoft YaHei'),
         local('PingFang SC'), local('Noto Sans CJK SC');
    unicode-range: U+2E80-9FFF, U+F900-FAFF, U+FE30-FE4F, U+3000-303F,
                   U+3040-30FF, U+31F0-31FF, U+AC00-D7AF, U+FF00-FFEF;
}}
...
html, body {{
    ...
    font-family: '{t['font_body']}', 'CJK Fallback', 'Helvetica Neue', Helvetica, Arial, {cjk_fallback}, sans-serif;
}}
.page {{
    position: relative;
    width: 794px; height: 1123px;
