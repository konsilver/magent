...
#         --content-json JSON      Inline JSON content (alternative to --content)
...
cmd_run() {
  ...
  # If --content-json was given, write it to a temp file and use as content_file
  if [[ -n "$content_json" && -z "$content_file" ]]; then
    echo "$content_json" > "$workdir/content_inline.json"
    content_file="$workdir/content_inline.json"
  fi
