...
# --- Detect pre-built DLL (Docker / CI scenario) ---
PREBUILT_DLL=""
for candidate in \
    "/opt/minimax-docx/MiniMaxAIDocx.Cli.dll" \
    "$SCRIPT_DIR/../bin/MiniMaxAIDocx.Cli.dll"; do
    if [ -f "$candidate" ]; then
        PREBUILT_DLL="$candidate"
        break
    fi
done

# --- Critical: .NET runtime / SDK ---
if ! command -v dotnet &>/dev/null; then
    printf "[FAIL]    %-14s not found\n" "dotnet"
    echo ""
    echo "  .NET runtime (or SDK) is REQUIRED. Install it:"
    case "$OS" in
    ...
    # Try SDK version first; fall back to runtime version detection
    local_ver=$(dotnet --version 2>/dev/null || echo "")
    if [ -n "$local_ver" ] && [ "$local_ver" != "0.0.0" ]; then
        local_major="${local_ver%%.*}"
        if [ "$local_major" -ge 8 ] 2>/dev/null; then
            printf "[OK]      %-14s %s SDK (>= 8.0)\n" "dotnet" "$local_ver"
        else
            printf "[FAIL]    %-14s %s (requires >= 8.0)\n" "dotnet" "$local_ver"
            STATUS="NOT READY"
        fi
    else
        # No SDK — check for runtime >= 8.0
        runtime_ver=$(dotnet --list-runtimes 2>/dev/null | grep -oP 'Microsoft\.NETCore\.App \K[0-9]+\.[0-9]+\.[0-9]+' | head -1 || echo "")
        if [ -n "$runtime_ver" ]; then
            runtime_major="${runtime_ver%%.*}"
            if [ "$runtime_major" -ge 8 ] 2>/dev/null; then
                printf "[OK]      %-14s %s runtime (>= 8.0)\n" "dotnet" "$runtime_ver"
            else
                printf "[FAIL]    %-14s runtime %s (requires >= 8.0)\n" "dotnet" "$runtime_ver"
                STATUS="NOT READY"
            fi
        else
            printf "[FAIL]    %-14s installed but no SDK or runtime >= 8.0 detected\n" "dotnet"
            STATUS="NOT READY"
        fi
    fi
fi

# --- Critical: Project build / pre-built DLL ---
if [ -n "$PREBUILT_DLL" ]; then
    printf "[OK]      %-14s pre-built at %s\n" "project" "$PREBUILT_DLL"
elif [ -d "$DOTNET_DIR" ]; then
    if [ -f "$DOTNET_DIR/MiniMaxAIDocx.Cli/bin/Debug/net10.0/MiniMaxAIDocx.Cli.dll" ] || \
       [ -f "$DOTNET_DIR/MiniMaxAIDocx.Cli/bin/Debug/net8.0/MiniMaxAIDocx.Cli.dll" ] || \
       [ -f "$DOTNET_DIR/MiniMaxAIDocx.Cli/bin/Release/net8.0/MiniMaxAIDocx.Cli.dll" ] || \
       [ -f "$DOTNET_DIR/MiniMaxAIDocx.Cli/bin/Release/net10.0/MiniMaxAIDocx.Cli.dll" ]; then
        printf "[OK]      %-14s built\n" "project"
    else
        # Try restore + build (requires SDK)
