...
_PLANNER_FIRST_PROMPT = """你是 Planner Agent，负责将用户任务拆解为一组线性、可执行的步骤。最后一个步骤的任务应该是整理总结前面步骤任务结果，用来回答用户任务

你的职责：定义「做什么」（宏观任务分解），不涉及「如何做」，不包含执行细节。同时结合用户特征与任务内容，生成一句话的用户目标（user_goal）作为本次计划的标题。
...
- 历史记忆是过去相似任务的计划方案，仅作参考，不得覆盖或替代 context.user 中的用户特征
...
{{
  ...
  "steps": [
    {{
      "step_id": 1,
      "brief_description": "步骤简述（≤15字，作为步骤标题）",
...
- brief_description 要简明扼要，不超过15字，作为该步骤的标题
- 每个 step 只描述「做什么」，不写「如何做」
- 不生成局部约束或输出格式
- **步骤解耦原则**：每个 step 应尽量独立，避免步骤之间强耦合；允许逻辑上的递进关系，但每步的输入应来自上下文而非对前一步的硬性依赖
- **步骤数量限制**：简单任务（问答、推荐、解释）2~3步；复杂任务最多不超过6步；禁止因过度拆分导致步骤冗余
- **【严格禁止】** 不得生成任何涉及"记录用户偏好/特征"、"保存用户信息"、"更新用户画像"、"记住用户偏好"等操作的步骤
- **complexity 分类规则**：
  - "simple"：信息检索、知识问答、内容推荐、解释概念、短文本生成（摘要/总结/翻译/格式转换）、列举/比较/选择——不需要调用外部工具、不需要执行代码
  - "complex"：代码生成或脚本编写（预期超过50行）、需要多步推理/数学计算/逻辑分析、需要调用工具（搜索/执行代码/文件操作）、方案设计/架构分析/长文档生成、综合汇总步骤（整合多步结果作为最终回答）"""


_PLANNER_REPLAN_PROMPT = """你是 Planner Agent，当前计划执行过程中某步骤失败，需要从指定步骤开始重新规划。最后一个步骤的任务应该是整理总结前面步骤任务结果，用来回答用户任务
...
注意：输出的 steps 只包含从 step_id={redo_id} 开始的新步骤，step_id 从 {redo_id} 开始编号。replan 不需要输出 user_goal（沿用原计划标题）。

规则：
- brief_description 要简明扼要，不超过15字，作为该步骤的标题
- 每个 step 只描述「做什么」，不写「如何做」
- 不生成局部约束或输出格式
- 必须利用 suggestion 做修正，避免重蹈覆辙
...
### Step 5：一致性自检
检查：
- 所有 local_constraint.target 是否都在 expected_output_schema.fields 中
...
## 验证流程（按顺序执行，发现错误后继续检查直到遍历完所有条目，不提前终止）
...
Step 5: 检查代码执行结果（仅当 SubAgent 执行结果中包含代码执行记录时）
- 若结果中明确提及 exit_code 非零（执行失败），则 → REDO，在 failure_reason 中说明代码执行失败原因和错误信息
- 若结果中未提及代码执行，跳过此步

注意：
- 遍历发现所有错误，全部写入 failure_reason 列表
- 内容基本达意、质量合理即视为合格，不要因格式细节或非关键字段缺失判定失败
- REPLAN 优先级高于 REDO
- 代码执行失败（exit_code != 0）属于硬性失败，不论其他约束是否满足均应判定 REDO
...
async def _run_qa(
    ...
    try:
        ...
        if data and "verdict" in data:
            ...
            logger.info("[QA] VERDICT=%s step=%s failure_reasons=%d is_last=%s",
                        verdict, step.step_id, failure_count, is_last_step)
    ...
    return {"verdict": "PASS", "failure_reason": [], "plan_suggestion": ""}
