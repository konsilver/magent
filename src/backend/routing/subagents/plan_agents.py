...
_PLANNER_FIRST_PROMPT = """你是 Planner Agent，负责将用户任务拆解为一组线性、可执行的步骤。只输出 JSON，不要其他内容。
...
## 规则
- 步骤数量：简单任务（问答/推荐/解释）2~3步；复杂任务最多6步
- 最后一步必须是整理汇总前序结果、回答用户问题
- complexity 分类：
  - simple：依赖模型自身知识即可完成（问答、总结、翻译、推荐、列举、比较）
  - complex：需借助外部工具（联网搜索、代码执行、文件操作）
- if_code_exc 分类：
  - true：该步骤核心职责是编写并生成新的可执行代码（必须包含验证执行环节，生成和执行集中在这一步骤，禁止生成其他额外验证该模块代码的步骤）
  - false：分析/阅读/整理/总结已有代码（该类任务禁止执行、验证代码），或非代码类任务；不确定时填 false
- 用户特征优先级高于历史记忆中的 suggestion
- 如果任务是需要生成代码且复杂的工程项目，则保证计划中每个会发生代码生成的步骤间代码可独立验证"""

_PLANNER_REPLAN_PROMPT = """你是 Planner Agent，当前计划某步骤执行失败，需要从指定步骤开始重新规划。只输出 JSON，不要其他内容。
...
## 重新规划指令
- 从 step_id={redo_id} 开始重新规划（之前已完成的步骤保持不变）
- 失败建议：{suggestion}
...
## 规则
- 输出的 steps 只包含从 step_id={redo_id} 开始的新步骤
- 最后一步必须是整理汇总前序结果、回答用户问题
- complexity / if_code_exc 规则同首次规划
- 用户特征优先级高于历史记忆中的 suggestion"""
    ...
    try:
        ...
        if data:
            ...
            if is_replan:
                ...
                kept_steps = [s for s in existing_steps if s.get("step_order", 0) < redo_id]
                new_steps = [
                    {
                        "step_id": s.get("step_id", f"{board['plan'].get('plan_id','plan')}_step_{redo_id + i}"),
                        "step_order": redo_id + i,
...
_WARMUP_PROMPT_TEMPLATE = """你是 Warmup Agent，任务是为整个计划制定全局约束，并为第一个步骤制定局部约束和输出格式。只输出 JSON，不要其他内容。

## 用户特征（context.user）
{user_context}

## 计划步骤（context.plan.steps）
{plan_steps}
...
**Step 1：判断任务风险等级**
- 低风险（问答/推荐/解释）→ global_constraints ≤ 2 条（soft为主），fields 可为空
- 中风险（数据分析/报告/方案设计）→ global_constraints ≤ 4 条，可含 hard 约束
- 高风险（代码执行/精确计算/外部工具）→ global_constraints 不限，hard 约束为主

**Step 2：生成 global_constraints**
- 约束必须可验证（"必须给出3条以上具体建议"优于"内容准确"）
- hard ≥ 60%，soft ≤ 40%；不要生成代码验证类约束


**Step 3：生成第一个步骤的 local_constraint**
- hard ≥ 60%，soft ≤ 40%；低风险禁止添加结构性约束
- 若第一步 if_code_exc=true，需为其添加验证代码执行效果的软约束
- 若步骤任务内容比较简单（不需要调用联网搜索、代码执行等工具就能完成），不要设置结构化约束
...
_QA_PROMPT_TEMPLATE = """你是 QA Agent，负责根据制定好的约束验证 SubAgent 的执行结果。不要调用任何工具，直接根据约束信息输出 JSON。
...
## 局部约束（local_constraint）
{local_constraint}

## 输出格式要求（expected_output_schema）
{expected_schema}

## 验证流程（按顺序执行，发现错误不提前终止，遍历完所有条目后汇总）

Step 1: 检查 expected_output_schema — 若 fields 为空则跳过；否则检查 required 字段是否存在 → 缺失则 REDO
Step 2: 检查所有 priority="hard" 的约束（global + local）→ 任意一条不满足则 REDO
Step 3: 检查所有 priority="soft" 的约束（global + local）,若任意一条confidence < 0.7→ 不满足则 REDO

## 输出要求（只输出 JSON，不要其他文字）
{{
  "verdict": "PASS|REDO",
  "suggestion": "错误原因或修正建议（通过时填空字符串）",
  "plan_suggestion": "针对整个计划的优化建议（仅最后一步填写，其余填空字符串）"
}}"""
...
async def _run_qa(
    ...
    """Run QA Agent, return verdict dict (PASS or REDO only)."""
    ...
    prompt = _QA_PROMPT_TEMPLATE.format(
        global_constraints=json.dumps(global_constraints, ensure_ascii=False, indent=2),
    ...
    try:
        text = await _call_llm_agent_cached(prompt, model_name, user_id, timeout=30, _agent_label="QA")
        data = _parse_json_output(text)
