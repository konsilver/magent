...
## 严格要求
- 【极其重要】你的输出必须直接是最终回答内容本身，不得有任何前置说明、分析过程、思考步骤
- 禁止输出"让我..."、"我需要..."、"根据..."、"首先分析..."等任何分析性或过渡性语句
- 禁止输出"关键要求："、"内容结构："、"工作流程："等任何大纲或规划性内容
...
你的输出应该直接从回答的第一个实质性句子开始，就像这样：
（错误示例）让我整理一下内容... → 禁止
（正确示例）Agent Harness 是一套标准化框架... → 正确"""


# 思考过程的标志性开头词（用于后处理过滤）
_THINKING_PREFIX_PATTERNS = re.compile(
    r"^(让我|我需要|我来|首先，让我|根据以上|根据提供|好的，|好，|现在，|"
    r"用户要求|用户希望|下面|接下来，让我|重新|我将|我会|我要|内容结构|关键要求)",
    re.MULTILINE,
)


def _strip_thinking_preamble(text: str) -> str:
    """过滤 LLM 在正式回答前输出的思考过程段落。

    思考型模型（如 DeepSeek-R1）有时不用 <think> 标签，而直接把思考过程
    以普通文本输出。这些思考段落通常在正式回答之前，以"让我..."、"我需要..."
    等分析性语句开头，最后才是真正的回答内容。

    策略：找到最后一个思考性段落，取其后面的内容作为正式回答。
    如果找不到明显的分界，返回原文。
    """
    ...
    for i, para in enumerate(paragraphs):
        ...
        if _THINKING_PREFIX_PATTERNS.match(first_line):
            last_thinking_idx = i
    ...
    # 取思考段落之后的内容
    answer_paragraphs = paragraphs[last_thinking_idx + 1:]
    if not answer_paragraphs:
        # 思考段落之后没有内容（整个输出都是思考），返回原文
        return text
...
async def _run_summary(
    ...
    try:
        ...
        text = _strip_thinking_preamble(text)
