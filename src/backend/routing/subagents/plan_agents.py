...
1. 结合用户特征与已制定好的计划步骤，生成一句话的用户目标（user_goal）
2. 为整个计划制定全局约束（global_constraints）
3. 为计划的第一个步骤制定局部约束（local_constraint）和输出格式（expected_output_schema）
...
async def _run_warmup(
    ...
    try:
        ...
        if data:
            ...
            logger.info("[Warmup] DONE user_goal_chars=%d global_constraints=%d",
                        len(data.get("user_goal", "")),
