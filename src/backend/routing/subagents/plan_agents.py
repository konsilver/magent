...
- 如果步骤是需要代码执行的任务，则在该任务描述中不仅要包含代码的生成，还要包含代码的测试验证（不要额外制造一个步骤专门验证代码）
...
- **complexity 分类规则（核心标准：是否需要调用外部工具或执行代码）**：
  - "simple"：信息检索、知识问答、内容推荐、解释概念、短文本生成（摘要/总结/翻译/格式转换）、列举/比较/选择、整合已有文本结果的汇总步骤——完全依赖模型自身知识，不需要调用外部工具、不需要执行代码
  - "complex"：代码生成或脚本编写、需要调用工具（联网搜索/执行代码/文件操作/调用API）、方案设计/架构分析——判断依据是步骤执行时必须借助外部工具或代码执行才能完成"""
...
- 低风险任务禁止添加结构性约束
- 如果这个步骤是需要书写代码的任务，则构造对代码模块的预期效果的约束
...
- 遍历发现所有错误，将错误原因或修正建议汇总写入suggestion字段
