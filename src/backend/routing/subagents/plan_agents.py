...
- **【严格区分】if_code_exc 字段规则**：
  - true：该步骤的核心职责是**编写并生成新代码**（包含生成+验证的步骤也算），例如：实现一个函数、编写脚本、生成可运行代码
  - false：该步骤是**分析、阅读、整理、总结、解释已有代码**，或者是非代码类任务（问答、搜索、汇总等）
  - 判断标准是”是否需要产出新的可执行代码”，而非”是否涉及代码相关话题”，不确定时优先填 false”””
  - **如果任务属于ture，则这个步骤必须添加验证代码执行的部分，且之后的步骤不要有验证这个模块代码的任务，即生成与验证代码必须在同一步骤**
"""
...
- 你**永远不需要调用任何工具**，你只需要根据给你的约束信息判断verdict
