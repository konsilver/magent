...
## 任务复杂度评估（必须先做）
在制定约束前，先判断任务类型：
- **简单信息类任务**：推荐、介绍、解释、列举、总结、回答问题等，不需要多步骤精确计算或外部系统交互
- **复杂执行类任务**：需要代码执行、数据处理、多步精确推理、调用外部工具等

**简单信息类任务的约束原则**：
- global_constraints 不超过 2 条，且必须是内容质量类约束（如"内容需准确"），禁止结构性约束
- next_step_instruction 中 local_constraint 的 priority 设为 "soft"（不得设为 "hard"）
- expected_output_schema 的 validation_rules 保持空数组 []，required 字段最多 1 项（如 "items"），fields 和 types 保持简单
- assumptions 不超过 2 条，保持显而易见的假设

**复杂执行类任务的约束原则**：
- 可以生成详细约束和精确 schema，但约束必须明确、可验证，不得相互矛盾

## 你的任务
1. 结合 context 黑板中的用户特征，在 Planner 定义的 user_goal 基础上进一步细化用户目标
2. 按照任务复杂度评估结果，制定合理的全局约束（不要过度约束）
3. 列出显式假设（仅列出真正重要的，避免臆造）
4. 为第一个 SubAgent 制定局部约束和输出结构（简单任务务必保持轻量）
...
async def _run_warmup(
    ...
    try:
        ...
        if data:
            ...
            # 写入任务复杂度，供 QA Agent 调整验证阈值
            if data.get("task_complexity"):
                board["check"]["task_complexity"] = data["task_complexity"]
            logger.info("[Warmup] refined_goal_chars=%d global_constraints=%d assumptions=%d task_complexity=%s",
