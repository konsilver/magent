...
input:
{
  //是否是整个任务完成后的全局检查，如果是则下面的输入内容为空，你的判断依据在context中
  "global": false,
...
4. 检查局部constraint中的soft部分，交给LLM judge，如果返回fail or low confidence（<0.6）则认为失败
...
//QA在整个plan执行完后检查：
根据context中的success_criteria对最终结果进行检查，硬约束违反则判定失败，软约束结合LLM judge
